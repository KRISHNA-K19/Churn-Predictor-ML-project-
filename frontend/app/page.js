...
export default function Home() {
  const [formData, setFormData] = useState({
    tenure: 12, MonthlyCharges: 80, TotalCharges: 960,
    Contract: 'Month-to-month', InternetService: 'Fiber optic',
    PaymentMethod: 'Electronic check', SeniorCitizen: 'No',
  });
          ...
          {['single', 'history'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 font-semibold border-b-2 transition-all ${
                activeTab === tab ? 'border-blue-400 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-300'
              }`}>
            ...
            <div className="lg:col-span-2 bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-8 border border-gray-700 shadow-2xl">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={handlePredict} disabled={loading}
                  ...
                  {loading ? <span className="flex items-center justify-center gap-2"><span className="animate-spin">⏳</span> Analyzing...</span> : '🔮 Predict Churn Risk'}
                </button>
                <button onClick={resetForm} className="px-6 bg-gray-700 hover:bg-gray-600 text-white font-semibold py-3 rounded-xl transition-all">↺ Reset</button>
              </div>
              {error && (
        ...
        {activeTab === 'history' && (
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-8 border border-gray-700 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={fetchHistory} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm transition-all">🔄 Refresh</button>
            </div>
            {loadingHistory ? (
              ...
              <div className="space-y-3">
                {predictions.map((pred, idx) => (
                  <div key={pred.id} className="bg-gray-700 rounded-lg p-4 flex items-center justify-between hover:bg-gray-600 transition-all">
                    <div>
